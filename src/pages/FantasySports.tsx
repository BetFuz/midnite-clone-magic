...
const FantasySports = () => {
  ...
  const topManagers = [
    ...
    { rank: 4, name: "Emma Wilson", team: "Victory United", points: 3354, profit: 4500000 },
    { rank: 5, name: "David Lee", team: "Champions FC", points: 3312, profit: 3000000 }
  ];
          ...
          <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6">
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground">Build your dream team and compete for prizes</p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <Card className="p-4">
                ...
                <p className="text-2xl font-bold text-green-500">{formatCurrency(255000000)}</p>
              </Card>
            </div>
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab}>
                  ...
                  <TabsContent value="football" className="space-y-4">
                    {leagues.filter(l => l.sport === "Football").map(league => (
                      <Card key={league.id} className={`p-6 ${league.myTeam ? 'border-primary/50 bg-primary/5' : ''}`}>
                          ...
                          <div className="flex-1">
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-4">
                              ...
                              <div>
                                ...
                                <p className="font-bold text-green-500">{formatCurrency(league.prizePool)}</p>
                              </div>
                              <div>
                                <p className="text-xs text-muted-foreground">Entry Fee</p>
                                <p className="font-bold">{formatCurrency(league.entryFee)}</p>
                              </div>
                              <div>
                                <p className="text-xs text-muted-foreground">Deadline</p>
                                <p className="font-bold text-orange-500">{league.deadline}</p>
                              </div>
                            </div>
                              ...
                              <div className="bg-muted/50 rounded-lg p-4 mb-4">
                                <div className="grid grid-cols-2 gap-4">
                                  <div>
                                    ...
                                    <p className="text-xl font-bold text-primary">#{league.myRank?.toLocaleString()}</p>
                                  </div>
                                  <div>
                                    <p className="text-xs text-muted-foreground">My Points</p>
                                    <p className="text-xl font-bold">{league.myPoints?.toLocaleString()}</p>
                                  </div>
                                </div>
                              </div>
                            )}
                        ...
                        <div className="flex gap-3">
                          {league.myTeam ? (
                            ...
                            <>
                              <Button className="flex-1">Join League</Button>
                    ...
                    {leagues.filter(l => l.sport === "Basketball").map(league => (
                      <Card key={league.id} className={`p-6 ${league.myTeam ? 'border-primary/50 bg-primary/5' : ''}`}>
                        <div className="flex items-center gap-2 mb-2">
                          <Badge variant="outline">{league.sport}</Badge>
                          <Badge variant="secondary">{league.season}</Badge>
                          {league.myTeam && <Badge className="bg-primary/10 text-primary">My Team</Badge>}
                        </div>
                        <h3 className="text-xl font-bold mb-4">{league.name}</h3>
                        <div className="grid grid-cols-2 gap-4 mb-4">
                          <div>
                            <p className="text-xs text-muted-foreground">Participants</p>
                            <p className="font-bold">{league.participants.toLocaleString()}</p>
                        ...
                        {league.myTeam && (
                          <div className="bg-muted/50 rounded-lg p-4 mb-4">
                            <p className="text-xs text-muted-foreground mb-1">My Rank</p>
                            <p className="text-2xl font-bold text-primary">#{league.myRank?.toLocaleString()}</p>
                          </div>
                        )}
                        <Button className="w-full">{league.myTeam ? 'Manage Team' : 'Join League'}</Button>
                      </Card>
                    ))}
                  </TabsContent>

                  <TabsContent value="cricket" className="space-y-4">
                    {leagues.filter(l => l.sport === "Cricket").map(league => (
                    ...
                    {topManagers.map(manager => (
                      <div key={manager.rank} className="flex items-center gap-3 p-3 rounded-lg hover:bg-muted/50 transition-colors">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${
                          ...
                          manager.rank === 3 ? 'bg-orange-500 text-white' :
                          'bg-muted'
                        }`}>
                          {manager.rank}
                        </div>
